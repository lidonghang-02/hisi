...
char udp_thread(void)
{
    ...
    while(1)
    {
        ...
        if(ret>0)
        {
            ...
            //启动STA模式
            errcode = start_sta_connect(ssid, strlen(ssid), passwd, strlen(passwd));
            if (errcode == 0)
                return 0;
		}
        ...
        if (overtime++ > 20)
        {
            printf("Litter Cleaner has not get any information about wifi\n");
            return -1;
        }
    }
}
...
int wifi_start_softap(void)
{
    ...
    rc = memcpy_s(hapd_conf.ssid, HI_WIFI_MAX_SSID_LEN + 1, "LitterCleaner", 16); /* 9:ssid length */
    if (rc != EOK) {
        return -1;
    }
